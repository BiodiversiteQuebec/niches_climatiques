...
#| tbl-cap: "Variables d'habitat utilisées pour chaque espèce"
...
sv <- data.frame(Espèce = names(species_vars), Variables = sapply(species_vars, paste, collapse = ", "))
...
x <- data.frame(Espèce = names(bp), Début = sapply(bp, "[", 1), Fin = sapply(bp, "[", 2))
rownames(x) <- 1:nrow(x)
